...
- Insert a header comment with the `sf-dev-kit:agent-spec` provenance + git SHA + iso timestamp
...
Final YAML shape:
```yaml
# Generated by /sf-dev-kit:agent-spec on 2026-04-28T17:00:00Z (sha: abc123)
...
- Run /sf-dev-kit:agent-spec --spec specs/agent-order_helper.yaml to
...
- **Cross-reference MCP tools.** If `/sf-dev-kit:agent-discover` lists an agent that already has a `case_create` tool bound, the new agent should reuse it rather than spawn a duplicate
...
- `/sf-dev-kit:agent-test` reads the spec to derive default eval cases
- `/sf-dev-kit:agent-deploy` cross-checks deployed metadata against the spec to flag drift
