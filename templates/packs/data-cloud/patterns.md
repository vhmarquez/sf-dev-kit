...
For agent actions, expose this via the MCP bridge (see SF-16 + `/sf-dev-kit:mcp-bridge`) so an Agentforce agent can ground responses against Data Cloud insights. Always honor `WITH USER_MODE` semantics — Data Cloud has its own permission model (Data Spaces, Data Cloud Permission Sets) that must be configured on the Connected App / running user.
